...
Goal state: seeded Fieldnote book, dev server on 43173, `npm test` showing exactly **1 failed / 36 passed**, no leftover personal rules from `/create-rule`, no leftover Canvas from the 101 beat, and no leftover Figma Slides deck from the MCP beat. Restore `lib/disputes/suggested-credit-api.ts` if a prior demo switched the client to v2.
