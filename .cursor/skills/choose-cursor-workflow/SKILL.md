...
3. **How does my agent get more information?** — Linear MCP, dsp_1043 suggested-credit issue on ce-field-demos, marketplace Linear plugin, `/standard-bug-fix` on the filter-pills issue. The operator creates a private Linear team in the UI first (Settings → Teams → New team, Make team private). Then run `stage-linear-201`. `.cursor/mcp.json` has no project servers. Do not stand up a SQLite MCP.
