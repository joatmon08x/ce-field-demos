...
- There is no project database MCP. Do not add `ledgerly-db` or query SQLite over MCP. Use Prisma here, `lib/data.ts`, or `curl` against `/api/invoices` and `/api/disputes`.
